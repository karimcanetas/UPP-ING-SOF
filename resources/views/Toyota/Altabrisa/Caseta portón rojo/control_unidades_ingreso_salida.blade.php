<form action="{{ route('incidencias.store') }}" method="POST" id="ingreso_salidabp_altabrisa">
    @csrf
        ...
        <div id="control_ingreso_salida" style="display: none;">
            ...
            <div class="card horizontal-card d-none">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="id_casetas" id="caseta_ingreso" readonly>
                </div>
                <div class="form-group">
                    <label for="id_turnos">Turno:</label>
                    <input type="text" class="form-control" name="id_turnos" id="turno_ingreso" readonly>
                </div>
                <div class="form-group">
                    <label for="Nombre_vigilante">Nombre de Vigilante:</label>
                    <input type="text" class="form-control" name="Nombre_vigilante" id="Nombre_vigilante_ingreso"
                        readonly>
                </div>
                <div>
                    <label for="fecha_hora">Fecha y hora del envio</label>
                    <input type="text" class="form-control fechahora" name="fecha_hora" id="fecha_hora_ingreso"
                        readonly>
                </div>
            </div>
            ...
            <div class="card horizontal-card d-none">
                <div class="form_group">
                    ...
                    <input type="text" class="form-control" name="id_formatos" id="formatos_ingreso" readonly>
                </div>
            </div>


            @foreach (['Numero de orden', 'Folio/Salida definitiva', 'Hora de Ingreso a la agencia', 'Unidad', 'VIN (6 últimos dígitos)', 'Color', 'Placas', 'Observaciones / Comentarios'] as $campoNombre)
                @if ($campo = $campos->firstWhere('campo', $campoNombre))
                    <div class="form-group">
                        <label for="campos[{{ $campo->id_campo }}]">{{ $campo->campo }}:</label>

                        @if ($campoNombre == 'Numero de orden')
                        ...
                        @elseif($campoNombre == 'Folio/Salida definitiva')
                            <input type="text" class="form-control" id="campos[{{ $campo->id_campo }}]"
                                name="campos[{{ $campo->id_campo }}]" value="{{ old('campos.' . $campo->id_campo) }}"
                                required>
                        @elseif($campoNombre == 'Hora de Ingreso a la agencia')
                        ...
                        @elseif ($campoNombre == 'Unidad')
                            ...
                            <div class="textunidad" style="display: none; margin-top: 10px;">
                                <label for="otrotextunidad4">Especificar otra unidad</label>
                                <textarea class="form-control otrotextunidad" id="campos[{{ $campo->id_campo }}]" name="campos[{{ $campo->id_campo }}]"
                                    rows="4" placeholder="Especifica otra unidad"></textarea>
                            </div>
                        ...
                        @elseif($campoNombre == 'Observaciones / Comentarios')
                            <textarea class="form-control" id="campos[{{ $campo->id_campo }}]" name="campos[{{ $campo->id_campo }}]"
                                rows="4">{{ old('campos.' . $campo->id_campo) }}</textarea>
                        @endif
                    </div>
                @endif
            @endforeach
