    ...
    <div class="text-center">
        ...
        <div class="btn-container" id="caseta-buttons">
            @forelse ($casetas as $caseta)
                <button class="btn pulse-effect incoming incoming-from-bottom"
                    onclick="showForm('{{ $caseta->nombre }}', {{ $caseta->id_casetas }})">
                    <i class="fas fa-door-open"></i> {{ $caseta->nombre }}
                </button>
            @empty
                <a class="btn pulse-effect incoming incoming-from-bottom" disabled>
